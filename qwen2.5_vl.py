...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def get_rope_index(
        ...
        **kwargs,
        ...
        position_ids = torch.zeros(     # [3, batch_size, sequence_length]
            3,
            input_ids.shape[0],
            input_ids.shape[1],
        ...
        grid_iters = {      # 按需取每张图/视频的网格
            1: iter(image_grid_thw) if image_grid_thw is not None else None,
            2: iter(video_grid_thw) if video_grid_thw is not None else None,
        }
        second_per_grid_ts = (         # 视频每帧几秒
            iter(second_per_grid_ts) if second_per_grid_ts is not None else iter([1] * input_ids.shape[1])
        )
        for batch_idx, current_input_ids in enumerate(input_ids):
            input_token_type = mm_token_type_ids[batch_idx]     # 每个 token 的模态:0文本/1图/2视频
            if attention_mask is not None:      # mask 掉 padding token
                current_input_ids = current_input_ids[attention_mask[batch_idx].bool()]
                input_token_type = input_token_type[attention_mask[batch_idx].bool()]

            input_type_group = []
            # 把 [0,0,0,1,1,1,1,0,0] 这样的模态序列切成 [(0,0,3), (1,3,7), (0,7,9)] —— 即"文本段、图像段、文本段"
            for key, group in itertools.groupby(enumerate(input_token_type.tolist()), lambda x: x[1]): # 按第二个元素分类
                group = list(group)
            ...
            for modality_type, start_idx, end_idx in input_type_group:
                ...
                # image == 1, video == 2
                    ...
                    grid_thw = next(grid_iters[modality_type])  # modality_type == 1, 2 图片，视频
                    # Only apply temporal scaling for videos; still images have no
                    # temporal dimension to space out (fixes #45325).
                    if modality_type == 2:
                        time_interval = tokens_per_second * int(next(second_per_grid_ts))   # tokens_per_second就是一个缩放因子
                    else:
                        time_interval = 1
                    ...
                    current_pos += max(grid_thw[1], grid_thw[2]) // spatial_merge_size  # 三维编码
            llm_positions = torch.cat(llm_pos_ids_list, dim=1).reshape(3, -1)   # [3, seq_len]
            if attention_mask is not None:
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        logits = self.lm_head(hidden_states[:, slice_indices, :])   # [batch_size, sequence_length, vocab_size]，只获取需要的logits
